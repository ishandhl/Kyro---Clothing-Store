...
async function startServer() {
  ...
  // In-memory/File-persisted Mock Database for Products, Orders, and Reviews
  ...
  // 2. API: Save/Update products (Admin panel integration)
  ...
  // 3. API: Delete product (Admin panel integration)
  ...
  // 4. API: Create or list orders with Stock Validation & Deduction
  ...
  // 4.5. API: Reviews Management (Fetch & Post)
  ...
  app.post("/api/reviews", (req, res) => {
    ...
    // Dynamic product rating recalculation on backend
  ...
  // 5. API: AI Stylist & Outfit Advisor using Gemini API
  app.post("/api/advisor", async (req, res) => {
    ...
    try {
      ...
      if (!apiKey || apiKey === "MY_GEMINI_API_KEY" || apiKey.trim() === "") {
        // Graceful error if API Key is not set up
        return res.json({
      ...
      // Initialize Gemini Client inside the request (lazy-loaded to prevent startup crashes)
      ...
      // Construct a pristine system instruction containing product context
...
3. To allow the UI to show interactive product tiles beneath your chat bubble, you MUST end your message with a structured recommendation block:
   ...
   Example: "For a muted modular tech-wear drape, I suggest layering our Technical Shell Jacket over the Heavy Loopback Hoodie, completed with the Modular Cargo Pants. [RECOMMENDATIONS: kr-04, kr-01, kr-02]"`;
      ...
      // Parse the custom recommendations tag [RECOMMENDATIONS: kr-01, kr-02]
      ...
      // Clean the tag out of the visible text so it looks seamless
      ...
      // Fallback rule-based styling matcher matching exact items in stock
      ...
      // Filter productIds to make sure they exist in db
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[KYRO SERVER] Running at http://localhost:${PORT}`);
  });
}
